(function(QUnit) {
    ...
    QUnit.test('has', function(assert) {
        ...
        var outsider = new Backbone.Model({
            id: 4
        });
    ...
    QUnit.test('create with validate:true enforces validation', function(assert) {
        ...
        assert.equal(collection.create({
            foo: 'bar'
        }, {
            validate: true
        }), false);
    ...
    QUnit.test('Underscore methods with object-style and property-style iteratee', function(assert) {
        ...
        assert.equal(coll.find({
            a: 0
        }), undefined);
        assert.deepEqual(coll.find({
            a: 4
        }), model);
        assert.equal(coll.find('d'), undefined);
        assert.deepEqual(coll.find('e'), model);
        assert.equal(coll.filter({
            a: 0
        }), false);
        assert.deepEqual(coll.filter({
            a: 4
        }), [model]);
        assert.equal(coll.some({
            a: 0
        }), false);
        assert.equal(coll.some({
            a: 1
        }), true);
        assert.equal(coll.reject({
            a: 0
        }).length, 4);
        assert.deepEqual(coll.reject({
            a: 4
        }), _.without(coll.models, model));
        assert.equal(coll.every({
            a: 0
        }), false);
        assert.equal(coll.every({
            b: 1
        }), true);
        assert.deepEqual(coll.partition({
            a: 0
        })[0], []);
        assert.deepEqual(coll.partition({
            a: 0
        ...
        assert.deepEqual(coll.partition({
            a: 4
        })[1], _.without(coll.models, model));
        assert.deepEqual(coll.map({
            a: 2
        }), [false, true, false, false]);
        assert.deepEqual(coll.map('a'), [1, 2, 3, 4]);
        assert.deepEqual(coll.sortBy('a')[3], model);
        assert.deepEqual(coll.sortBy('e')[0], model);
        assert.deepEqual(coll.countBy({
            a: 4
        }), {
            'false': 3,
            'true': 1
        });
        assert.deepEqual(coll.countBy('d'), {
            'undefined': 4
        });
        assert.equal(coll.findIndex({
            b: 1
        }), 0);
        assert.equal(coll.findIndex({
            b: 9
        }), -1);
        assert.equal(coll.findLastIndex({
            b: 1
        }), 3);
        assert.equal(coll.findLastIndex({
            b: 9
    ...
    QUnit.test('falsy comparator', function(assert) {
        ...
        var collection = new Col();
        var colFalse = new Col(null, {
            comparator: false
        });
        var colNull = new Col(null, {
            comparator: null
        });
        var colUndefined = new Col(null, {
    ...
    QUnit.test('#2428 - push duplicate models, return the correct one', function(assert) {
        assert.expect(1);
        var collection = new Backbone.Collection;
        var model1 = collection.push({
            id: 101
        });
        var model2 = collection.push({
            id: 101
        });
    ...
    QUnit.test('#2612 - nested `parse` works with `Collection#set`', function(assert) {
        ...
        var Item = Backbone.Model.extend({
            ...
            parse: function(attrs) {
                this.subItems.set(attrs.subItems, {
                    parse: true
                });
    ...
    QUnit.test('modelId', function(assert) {
        ...
        assert.equal(StoogeCollection.prototype.modelId({
            id: 1
        }), 1);
    ...
    QUnit.test("#3711 - set's `update` event returns one merged model", function(assert) {
        ...
        var model2Update = new Backbone.Model({
            id: 2,
    ...
    QUnit.test("#3711 - set's `update` event returns multiple merged models", function(assert) {
        ...
        var modelUpdate = new Backbone.Model({
            id: 1,
            title: 'First Post V2'
        });
        var model2 = new Backbone.Model({
