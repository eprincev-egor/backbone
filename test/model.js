(function(QUnit) {
    ...
    QUnit.test('underscore methods', function(assert) {
        ...
        assert.deepEqual(model.invert(), {
            a: 'foo',
            b: 'bar',
            c: 'baz'
        });
        assert.deepEqual(model.pick('foo', 'baz'), {
            foo: 'a',
            baz: 'c'
        });
        assert.deepEqual(model.omit('foo', 'bar'), {
            baz: 'c'
        });
    });
    ...
    QUnit.test('matches', function(assert) {
        ...
        assert.strictEqual(model.matches({
            name: 'Jonas',
            cool: true
        }), false);
    ...
    QUnit.test('change, hasChanged, changedAttributes, previous, previousAttributes', function(assert) {
        ...
        model.on('change', function() {
            ...
            assert.ok(_.isEqual(model.changedAttributes(), {
                name: 'Rob'
            }), 'changedAttributes returns the changed attrs');
            assert.equal(model.previous('name'), 'Tim');
            assert.ok(_.isEqual(model.previousAttributes(), {
                name: 'Tim',
                age: 10
    ...
    QUnit.test('changedAttributes', function(assert) {
        ...
        assert.deepEqual(model.changedAttributes(), false);
        assert.equal(model.changedAttributes({
            a: 'a'
        }), false);
        assert.equal(model.changedAttributes({
            a: 'b'
    ...
    QUnit.test('`save` with `wait` sends correct attributes', function(assert) {
        ...
        assert.deepEqual(JSON.parse(this.ajaxSettings.data), {
            x: 3,
            y: 2
        });
        assert.equal(model.get('x'), 1);
    ...
    QUnit.test("nested `set` during `'change'`", function(assert) {
        ...
        model.on('change', function() {
            switch (count++) {
                case 0:
                    assert.deepEqual(this.changedAttributes(), {
                        x: true
                    });
